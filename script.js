document.addEventListener("DOMContentLoaded", () => {
 ...
 addTaskBtn.addEventListener("click", () => {
   ...
   logHistory("Added task", "New Task"); // Add log entry here
 });
   ...
   editBtn.addEventListener("click", () => {
     if (taskTitle.readOnly) {
       ...
       if (taskTitle.value.trim() === "") {
         ...
         logHistory("Edited task", task.title);
       }
     }
   });
 ...
 function createSubtaskElement(task, subtask, taskIndex, subtaskIndex) {
   ...
   subtaskTitle.style.textDecoration = subtask.completed
     ? "line-through"
     : "none";
   ...
   editSubtaskBtn.addEventListener("click", () => {
     if (subtaskTitle.readOnly) {
       ...
       if (subtaskTitle.value.trim() === "") {
         ...
         logHistory("Edited subtask", subtask.title);
       }
     }
   });
   ...
   completeSubtaskBtn.addEventListener("click", () => {
     ...
     completeSubtaskBtn.textContent = subtask.completed ? "Undo" : "Complete";
     logHistory(
       subtask.completed ? "Completed subtask" : "Uncompleted subtask",
       subtask.title
     );
 ...
 function updateProgress() {
   const totalItems =
     tasks.length + tasks.reduce((acc, task) => acc + task.subtasks.length, 0);
 ...
 function updateDueDate(taskId, dueDate) {
   ...
   logHistory(
     "Updated due date",
     `Task: ${
       taskElement.querySelector(".task-content").textContent || "Unnamed Task"
     }, Due: ${dueDate}`
   );
 }
});
